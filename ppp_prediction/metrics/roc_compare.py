...
def delong_roc_test(ground_truth, predictions_one, predictions_two):
    ...
    aucs, delongcov = fastDeLong(predictions_sorted_transposed, label_1_count)
    log10p = calc_pvalue(aucs, delongcov)
    return log10p
...
def roc_test(y_true, y_ref, y_new, method="delong"):
    ...
    if method == "delong":
        ...
        aucs, delongcov = fastDeLong(predictions_sorted_transposed, label_1_count)
        log10p = calc_pvalue(aucs, delongcov)
        p = np.power(10, log10p).item()
        auc_diff = aucs[1] - aucs[0]
        se = np.sqrt(delongcov[0, 0] + delongcov[1, 1] - 2 * delongcov[0, 1])
        auc_diff_UCI = auc_diff + 1.96 * se
        auc_diff_LCI = auc_diff - 1.96 * se
        # se
        return {
            "AUC_diff": auc_diff,
            "AUC_diff P-value": p,
            "AUC_diff LCI": auc_diff_LCI,
            "AUC_diff UCI": auc_diff_UCI,
        }
    else:
        raise NotImplementedError("Only DeLong method is implemented")