...
AgeSex      xgboost        NaN  
    ...
    def update_model(self, new_model_table=None, duplicate_replace=False):
        ...
        for name, model_table_row in self.model_table.iterrows():
            ...
            try:
                status = model_table_row["status"]["fitted"]

                if status == 1:
                    logging.warning(f"{name} already fitted, will skip it")
                    continue
            except KeyError:
                # raise ValueError(f"{name} have no status, will skip it")
                pass

            # unpack the params

            params = model_table_row["param"].to_dict()
            # get load_local_cache
            load_local_cache = params.get("load_local_cache", True)
            ...
            self.fit(
                ...
                model_fn=model_fn,
                load_local_cache=load_local_cache,
                **model_fn_config,
            )
    ...
    def fit(
        ...
        **model_fn_config,
        ...
        # step2 fit the model
        # save model from local
        if self.saveDir is not None:
            modelSaveDir = Path(self.saveDir) / f"{score_name}.pkl"
            trainScoreSaveDir = Path(self.saveDir) / f"{score_name}_train.feather"
            testScoreSaveDir = Path(self.saveDir) / f"{score_name}_test.feather"

        need_to_fit_flag = True
        if self.saveDir is not None and load_local_cache:
            if modelSaveDir.exists():
                model = pickle.load(modelSaveDir.open("rb"))
                need_to_fit_flag = False
                logging.info(f"Load model from {modelSaveDir} of {score_name}")

        if need_to_fit_flag:
            logging.info(f"Start to fit {name} with {model_fn.__name__}")
            model, *_ = model_fn(
                train=self.train,
                # test=self.test,
                xvar=xvar,
        ...
        re_cal_score_flag = True
        if self.saveDir is not None and load_local_cache:
            if trainScoreSaveDir.exists() and testScoreSaveDir.exists():
                train_score = pd.read_feather(trainScoreSaveDir)
                test_score = pd.read_feather(testScoreSaveDir)
                self.train_score = self.train_score.merge(train_score, on="eid")
                self.test_score = self.test_score.merge(test_score, on="eid")

                re_cal_score_flag = False
                logging.info(
                    f"Load score from {trainScoreSaveDir} and {testScoreSaveDir}"
                )

        if re_cal_score_flag:
            self.train_score[score_name] = get_predict_v2_from_df(
                model, self.train, xvar
            )
            self.test_score[score_name] = get_predict_v2_from_df(model, self.test, xvar)
            if self.saveDir is not None:
                self.train_score[["eid", score_name]].reset_index(drop=True).to_feather(
                    trainScoreSaveDir
                )
                self.test_score[["eid", score_name]].reset_index(drop=True).to_feather(
                    testScoreSaveDir
                )
                logging.info(
                    f"Save score to {trainScoreSaveDir} and {testScoreSaveDir}"
                )

        ## add the score into train_score
        self.train = self.train.merge(self.train_score[["eid", score_name]], on="eid")
        self.test = self.test.merge(self.test_score[["eid", score_name]], on="eid")
        ...
        if self.saveDir is not None and not modelSaveDir.exists():
            pickle.dump(model, open(modelSaveDir, "wb"))
