...
glmnet_cox <-
  function(train, test, xvar, statu, time, cv, save_dir) {
    library(glmnet)
    library(survival)
    library(ggplot2)
    library(export)
    ...
    write.csv(train$risk_score,
              paste0(save_dir, "/train_risk_score.csv"),
              row.names = FALSE)
    write.csv(test$risk_score,
              paste0(save_dir, "/test_risk_score.csv"),
              row.names = FALSE)
    return (list(train = train, test = test, model = cvfit))
  }
